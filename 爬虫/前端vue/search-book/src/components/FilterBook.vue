<template>
  <div class="container sep15-">
    <div class="search-toolbox">
      <div class="sep sep1">
        <b>类型：</b>
        <a class="on sep1" rel="nofollow" data="f-" isAll>全部</a>
        <span class="sep">|</span>
      ...
      <div class="sep sep2">
        <b>筛选：</b>
        <a class="on sep2" rel="nofollow" data="nd-" isAll>全部</a>
        <a rel="nofollow" data="nd-d">只看目录，去除文件</a>
        <a rel="nofollow" data="nd-f">只看文件，去除目录</a>
      </div>
      <div class="sep sep3">
        <b>年份：</b>
        <a class="on sep3" rel="nofollow" data="nd-" title="只看全部年分享的资源" isAll>全部</a>
      ...
      <div class="sep sep4">
        <b>排序：</b>
        <a class="on sep4" rel="nofollow" data="s-" isAll>默认</a>
        <span class="sep">|</span>
  ...
  mounted() {
    ...
    document.querySelectorAll(".search-toolbox a").forEach(element => {
      element.addEventListener("click", function() {
        ...
        document.querySelector(selectClass).classList.remove("on", parentClass);
  ...
  watch: {
    listData(newVal, oldVal) {
      ...
      if (isSearch) {
        ...
        document.querySelectorAll(".search-toolbox a").forEach(element => {
          let parentClass = this.parentNode.classList[1];
          if (this.getAttribute("isAll")) {
            this.classList.add("on", parentClass);
          } else {
            this.classList.remove("on", parentClass);
          }
        });
      }
    }
